...
const c = await fetch(`${ORG}/api/data/v9.2/customcontrols?$filter=name eq 'vip_vip.KustoExplorer'&$select=customcontrolid,name,version,manifest`,{headers:H}).then(r=>r.json());
console.log('count:', c.value?.length);
if (c.value?.[0]) console.log('id:', c.value[0].customcontrolid, 'version:', c.value[0].version);
