...
// load some org-version metadata
const o = await fetch(`${ORG}/api/data/v9.2/organizations?$select=organizationid,versionnumber,sqlaccessgroupversionnumber`,{headers:H}).then(r=>r.json());
console.log('org:', JSON.stringify(o.value, null, 2));
// fetch ALL webresources containing KustoExplorer name
const wrAll = await fetch(`${ORG}/api/data/v9.2/webresourceset?$filter=contains(name,'KustoExplorer')&$select=name,versionnumber,modifiedon`,{headers:H}).then(r=>r.json());
console.log('webresources:', JSON.stringify(wrAll.value, null, 2));
