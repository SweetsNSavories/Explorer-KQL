...
const wr = await fetch(`${ORG}/api/data/v9.2/webresourceset?$filter=name eq 'cc_vip.KustoExplorer/bundle.js'&$select=webresourceid,name,content`,{headers:H}).then(r=>r.json());
console.log('count:', wr.value?.length);
if (wr.value?.[0]) {
  const w = wr.value[0];
  const len = w.content ? Buffer.from(w.content,'base64').length : 0;
  console.log('id:', w.webresourceid, 'len:', len);
  const txt = w.content ? Buffer.from(w.content,'base64').toString('utf8') : '';
  console.log('hasMonacoEditor:', txt.includes('monaco-editor'), 'hasKxpMonaco:', txt.includes('kxp-monaco'));
}
