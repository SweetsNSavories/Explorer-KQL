...
const r=await fetch(`${ORG}/api/data/v9.2/customcontrols?$filter=name eq 'vip.KustoExplorer'`,{headers:H});
const j=await r.json();
console.log('raw:', JSON.stringify(j).slice(0,500));
for(const cc of (j.value||[])){
  console.log('cc id', cc.customcontrolid, 'name', cc.name, 'publisher', cc._publisherid_value);
  // resolve publisher
  const pr=await fetch(`${ORG}/api/data/v9.2/publishers(${cc._publisherid_value})?$select=uniquename,customizationprefix`,{headers:H});
  const pj=await pr.json();
  console.log('cc',cc.customcontrolid,'publisher',pj.uniquename,'prefix',pj.customizationprefix);
  if(pj.customizationprefix==='vip'){
    const d=await fetch(`${ORG}/api/data/v9.2/customcontrols(${cc.customcontrolid})`,{method:'DELETE',headers:H});
    console.log('delete cc',cc.customcontrolid,':',d.status, await d.text());
  }
}
