...
const ccId='d0ec2473-8fcb-4153-bed6-c73be0d0a2ed';
const r=await fetch(`${ORG}/api/data/v9.2/solutioncomponents?$filter=objectid eq ${ccId}`,{headers:{Authorization:'Bearer '+tok}});
const j=await r.json();
console.log('solutioncomponents for cc:', JSON.stringify(j,null,2));
// also check pluginpackage / publishedon
const r2=await fetch(`${ORG}/api/data/v9.2/customcontrols(${ccId})?$select=publickeytoken,solutionid,pcfprojecttoken,version,manifest`,{headers:{Authorization:'Bearer '+tok}});
console.log('cc detail:', JSON.stringify(await r2.json(),null,2));
